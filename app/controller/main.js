...
class MainController extends Controller {
  async nodeRank() {
    ...
    if (node_type === '1' && JSON.parse(isScore)) {
      sql = `
            SELECT nickname, address, type, ((lock_num * .8 + tickets * .2) * 100) as score, create_time, is_eligibility FROM team
            WHERE
            node_type=1
            AND
      ...
      sql = `
            ...
            AND is_eligibility='${isEligibility}'
            ORDER BY tickets
            DESC , create_time ASC
  ...
  async getMemberStatus() {
    ...
    const signUpEndTime = 1534348800000;

    if (signUpEndTime < +new Date() && Object.assign({}, data[0], type[0]).status !== 2) {
      ctx.body = {
        status: 403,
        message: '报名截止',
        data: {
