<template>
  <div class="power-container">
    <div class="page">
      <h2 class="header">{{ $t(`center.power.title`) }}</h2>
      <ul class="tabs">
        <li class="active">
          TRUE{{ $t(`center.power.subhead`) }}
        </li>
      </ul>
            ...
            <tbody>
              <template v-if="powerList.length">
                <tr v-for="(item, index) in powerList" :key="index">
                  ...
                  <td>{{item.status === '1' ? parseInt((+new Date() - (+item.create_time)) / 1000 / 60 / 60 / 24) : '0'}} {{ $t(`center.power.day`) }}</td>
...
.power-container {
  ...
  *:focus {
  ...
  .ps>.ps__rail-x:active>.ps__thumb-x, .ps>.ps__rail-x:hover>.ps__thumb-x {
    height: 8px;
  }
